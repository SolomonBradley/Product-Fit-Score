...
export function FitScoreCard({ result: initialResult }: { result: FitScoreResult }) {
  ...
  return (
    <div className="bg-card text-card-foreground rounded-xl border border-border shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <div className="grid md:grid-cols-2 gap-px bg-border">
        ...
        {boostMutation.isPending && (
          <div className="absolute inset-0 z-50 bg-background/60 backdrop-blur-sm flex items-center justify-center animate-in fade-in duration-300">
            <div className="flex flex-col items-center gap-4">
              <div className="w-12 h-12 rounded-full border-4 border-primary border-t-transparent animate-spin" />
              <p className="font-bold text-lg animate-pulse">Recalculating...</p>
              <p className="text-sm text-muted-foreground">Adjusting for your priorities</p>
            </div>
          </div>
        )}
      </div>

      {(result.boostableFeatures && result.boostableFeatures.length > 0) && (
        <div className="p-6 bg-primary/5 border-t border-primary/10 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {result.boostableFeatures.map(feature => (
              <Button 
                key={feature} 
                variant="outline" 
                size="sm" 
