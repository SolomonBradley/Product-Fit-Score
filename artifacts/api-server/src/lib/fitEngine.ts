import { isOllamaAvailable, generateJSON } from "./ollamaClient";
...
function getFilteredContext(user: UserContext, productCategory: string) {
    ...
    new Set(
      user.interests
        ...
        .flatMap((i) => {
          ...
          return ALL_KNOWN_CATEGORIES.filter(
            (kw) => !cat.includes(kw) && iL.includes(kw),
          );
        }),
    ),
  );
  ...
  const relevantOrders =
    user.recentOrders?.filter((o) => {
      const p = o.product.toLowerCase();
      if (
        (cat.includes("bag") || cat.includes("luggage")) &&
        (p.includes("bag") ||
          p.includes("luggage") ||
          p.includes("handbag") ||
          p.includes("wallet"))
...
function cleanPageContent(html: string): string {
  ...
  const breadcrumbMatch = html.match(
    /<div[^>]*id="wayfinding-breadcrumbs_container"[^>]*>([\s\S]*?)<\/div>/i,
  );
...
async function analyzeProduct(
  ...
  url: string,
  user: UserContext,
): Promise<MasterIntelligence | null> {
  const cleaned = cleanPageContent(html);

  // ─── Category Isolation Phase ─────────────────────────────────────────────
  const productCategory = cleaned.match(/CATEGORY: (.*)/)?.[1] || "General";
  const { filteredInterests, excludedCategories, relevantOrders } =
    getFilteredContext(user, productCategory);
  ...
  const orderList =
    relevantOrders
      .map((o) => `(${o.count}x, ${o.product}, ${o.source})`)
  ...
  const shoppingBrands =
    user.emailBrands?.filter((b) => {
      const lower = b.name.toLowerCase();
      return !NON_RETAIL_KEYWORDS.some((kw) => lower.includes(kw));
    }) || [];

  const brandList =
    shoppingBrands.length > 0
      ? shoppingBrands
          .map((b) => `${b.name} (${b.count} orders via ${b.source})`)
          .join(", ")
...
This analysis is ONLY about: ${productCategory}. Do NOT reference, compare, or mention user habits from any other category.`
    : `[SCOPE]
...
Return JSON ONLY:
{
  ...
  logger.info(
    ...
    "[Engine] Initiating LLM Analysis Mode (Strict Skeptical Logic)...",
  );
...
function getProceduralAnalysis(
  ...
  const category =
    cleaned.match(/CATEGORY: (.*)/)?.[1]?.toLowerCase() || "general";
  const { filteredInterests, relevantOrders } = getFilteredContext(
    user,
    category,
  );
  ...
  const scores = {
    ...
    productMatch: relevantOrders.some((o) =>
      title.includes(o.product.toLowerCase()),
    )
      ? 25
      : 0,
    interestMatch: filteredInterests.some((i) =>
      title.includes(i.toLowerCase()),
    )
      ? 15
      : 0,
  };
  ...
  if (scores.brandMatch && scores.productMatch) {
    whyItFitsYou = [
      "Direct Match: You frequently buy this brand and product type.",
    ];
  } else if (scores.productMatch) {
    whyItFitsYou = [
      "System Match: You have a documented history of buying this type of product.",
    ];
  }
...
export async function analyzeProductForUser(url: string, user: UserContext) {
  ...
  if (prioritizedFeatures.length < 2) {
    ...
    boostableFeatures = allFeatures.filter(
      (f) =>
        !prioritizedFeatures
          .map((p) => p.toLowerCase())
